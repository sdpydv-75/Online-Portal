    ...
    <nav style={{ background: 'rgba(10, 15, 26, 0.85)', backdropFilter: 'blur(20px)', padding: '0 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.06)', position: 'sticky', top: 0, zIndex: 100, height: '78px', display: 'flex', alignItems: 'center', boxShadow: '0 4px 30px rgba(0,0,0,0.3)', transition: 'all 0.3s ease' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', maxWidth: '1300px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
          ...
          <div className="desktop-nav" style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginLeft: '1rem', height: '100%' }}>
            {/* Home Link */}
        ...
        <div className={`nav-right ${isMenuOpen ? 'mobile-open' : ''}`} style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexShrink: 0 }}>
          {user ? (
            <>
              {user.role === 'student' && (
                <>
                  <Link to="/jobs" onClick={() => setIsMenuOpen(false)} style={{ textDecoration: 'none', color: '#94a3b8', fontWeight: '500', fontSize: '0.95rem', transition: 'color 0.2s' }}>Dashboard</Link>
